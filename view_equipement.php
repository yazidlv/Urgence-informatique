...
// permet d'afdficher la liste de l'équipement.
   ...
   foreign key (id_contrat) references contrat (id_contract))');
	
   
	$query= 'select id_mat, reference, nom, id_contrat from materiel order by nom';
	echo"<table width='0%' cellpadding='5' cellspacing='1' border='0' align='center'>"; 
	echo '<tr class="survolOrange fondPardefautColor">'; 
    echo '<ul><a name="retour" onclick="document.location=\'equipement.php?\' ;" /><img src="image/retour.png"/> </a></ul><br>';
	...
	if($res)
	{
	// je dévise $res dans un tableau.
		foreach($res as $row) 
		{   if ($row['reference']!== null && $row['nom']!== null)
	        {
