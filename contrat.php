...
<html>
	<head>
        ...
        <script type="text/javascript">
		    ...
		    // faire en sorte que lorsque l'on change le texte du champ de recherche, on affiche la boîte de suggestions
		     $('[name="query3"]').keyup(function(event){
				...
				$('#querySuggestions').on('click', 'li', function(){
					$('[name="query3"]').attr('value', $(this).text());
					hideSuggestionsBox();
				});
			});
	...
	<body>
		...
		<form id="queryForm" action="resultat.php" method="get">
			<input type="text" name="query3" maxlength="37">
...
$role= $user->getRole();	*/
   ...
   foreign key contrat(id_client) references  users(id))');
	
    echo '<ul><tr><a name="retour" onclick="document.location=\'principal.php?\'" /><img src="image/retour.png"/></a></tr><br></ul>';
 
   echo'<ul><a class ="info" style="target-name:new; target-new:tab;" target="_new" href="add_contrat.php"><img src="image/incident2.png"/><span>ajouter un contrat</span></a>
<a class="info" style="target-name:new; target-new:tab;" target="_new" href="view_contrat.php"><img src="image/contrat.png"/><span>visualiser la liste des contrats</span></a></ul><br><br>';
